...
// Copyright (c) 2014 The Quarkbar developers
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "quarkbar.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No quarkbar.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override quarkbar.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "quarkbard.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Quarkbar will not work properly.");
